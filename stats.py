...
bins = int(CURRENT_TIMESTAMP - CUTOFF_TIMESTAMP) // REQUEST_COUNT_RESOLUTION
...
y.append(y[-1])
spoilers_today = sum(1 for timestamp in spoilers if timestamp >= CUTOFF_TIMESTAMP)

# plot data
ax2 = ax1.twinx()
ax2.xaxis.set_major_formatter(md.DateFormatter('%H:%M'))
ax2.plot(x, y, color='xkcd:red')
ax2.axis(xmin=CUTOFF_TIME, xmax=CURRENT_TIME, ymin=total_spoilers - spoilers_today - 1)
